...
class WebObject(object):
    ...
    def page(self, agg_date=None, agg_nodes=None):
        f = open(self._get_main_name(), 'w')
        d = self.path

        message = self._main_head()
        ...
        for item in d['children']:  # for each main directory dstat-hadoop-cloud-x ...
            if item['type'] == 'directory' and item['name'] == 'aggregation':
                ...
                for idx, device in enumerate(item['children']):  # for each device i.e. cpu, network, memory
                    if device['type'] == 'directory':
                        ...
                        for image in device['children']:  # for each image inside the device dir
                            ...
                            if image['type'] == 'file' and image['name'].endswith('stacked.png'):
                                ...
                                if device['name'] == 'cpu':
                                    message += '<br><a href="' + \
                                               self._get_cpu_name(item['name'].split("-")[-1]) + \
                                               ' ">go to nodes page</a>'
                                elif device['name'] == 'disk':
                                    message += '<br><a href="' + self._get_disk_name(item['name'].split("-")[-1]) + \
                                               ' ">go to disks page</a>'
                                message += '</div>'
                                img_counter += 1
                message += "</div>"
    ...
    def _create_aggregation_page(self, basedir, exp='', agg_date=None, nodes=None):
